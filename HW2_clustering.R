...
# Features used for clustering (columns 3-6):
...
for (k in 1:20) {
  set.seed(1801)
  wcss_raw[k] <- sum(kmeans(penguins[, 3:6], centers = k)$withinss)
}
...
# --- K-means with k = 3 ---
set.seed(1801)
km_raw <- kmeans(penguins[, 3:6], centers = 3, nstart = 15)
...
penguins_scaled <- as.data.frame(lapply(penguins[, 3:6], min_max_norm))
...
plot(peng_hclust,
     ...
     xlab   = "",
     sub    = "",
     labels = FALSE,   # suppress individual labels for readability
     hang   = -1)
